...
def semisupervised_train(x_train, y_train, x_val, y_val, x_test, y_test, opt,config):
...
def supervised_train(x_train, y_train, x_val, y_val, x_test, y_test, opt,config):
    # construct data loader
    # Those are the transformations used in the paper
    prob = 0.2  # Transform Probability
    cutout = transforms_ts.Cutout(sigma=0.1, p=prob)
    ...
    partial_size = int(config['data_params']['label_ratio'] * train_dataset_size)
    train_ids = list(range(train_dataset_size))
    np.random.shuffle(train_ids)
    train_sampler = SubsetRandomSampler(train_ids[:partial_size])
    ...
    for epoch in range(opt.epochs):
        ...
        print('[Test-{}][{}][{}] Val ACC:{:.2f}%, Best Test ACC.: {:.2f}% in Epoch {}'.format(
        epoch,config['data_params']['label_ratio'],
            config['data_params']['alpha'], val_acc, test_acc, best_epoch))
        if opt.model_name=='supervised':
            early_stopping(val_acc, backbone_lineval,linear_layer,False)
        if early_stopping.early_stop:
            print("Early stopping")
            break
